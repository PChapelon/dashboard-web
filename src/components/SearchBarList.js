...
function renderItem (item, props) {
  return (<label className={(props.state.nbSelectedCountries >= 5 && !item.checked) ? 'searchBarList__item disabled' : 'searchBarList__item'} key={item.numericCode} for={'searchBarItem__' + item.numericCode}>
    ...
    <input id={'searchBarItem__' + item.numericCode} disabled={(props.state.nbSelectedCountries >= 5 && !item.checked) ? 'disabled' : ''} onchange={ () => { props.actions.toggleCountry(item.numericCode); props.actions.countSelectedCountries(); props.actions.sortBy({property: props.state.order.lastProperty, sameOrder: true}) }} type='checkbox' checked={item.checked ? 'checked' : ''}/>
